...
object RowToRowDescriptionMatcher {
  ...
  def apply(rowDescriptionXmlElem: Elem)
    : Either[UnrecoverableError, RowToRowDescriptionMatcher] =
    if (XMLHelperFunctions.haveLabel(rowDescriptionXmlElem,
                                     fixedOrderedRowDescriptionElemLabel)) {
      makeFixedRowToRowDescriptionMatcher(rowDescriptionXmlElem)
    } else if (XMLHelperFunctions.haveLabel(
                 rowDescriptionXmlElem,
                 delimitedOrderedRowDescriptionElemLabel)) {
      makeDelimitedRowToRowDescriptionMatcher(rowDescriptionXmlElem)
    } else {
      RowToRowDescriptionMatcherErrorMessages.unknownRowDescriptionElem(
        rowDescriptionXmlElem)
    }

  def makeDelimitedRowToRowDescriptionMatcher(rowDescriptionXmlElem: Elem)
    : Either[UnrecoverableError, RowToRowDescriptionMatcher] = {
    ...
    @tailrec def makeDelimitedColumnIdentifiersAndDelimitedColumnDescriptions(
        ...
        columnIdentifierList: List[DelimitedColumnIdentifier]
              ...
              (List[DelimitedColumnDescription],
               List[DelimitedColumnIdentifier])] = xmlNodes match {
      case elem :: _
          if !XMLHelperFunctions.elemLabelIsIn(
            elem,
            RowToRowDescriptionMatcher.columnDescriptionElemLabel,
            RowToRowDescriptionMatcher.columnIdentifierElemLabel
          ) =>
        RowToRowDescriptionMatcherErrorMessages
          .unknownDelimitedRowDescriptionChildElem(elem)

      case Nil =>
        Right((columnDescriptionList, columnIdentifierList))

      case elem :: restOfNodes
          if XMLHelperFunctions.haveLabel(
            elem,
            RowToRowDescriptionMatcher.columnDescriptionElemLabel) =>
        ...
        columnDescriptionEither match {
          ...
          case Right(columnDescription) =>
            makeDelimitedColumnIdentifiersAndDelimitedColumnDescriptions(
              restOfNodes,
    ...
    val unknownAttributes = XMLHelperFunctions.collectUnknownAttributes(
      delimitedOrderedRowDescriptionAttributeList,
    ...
    unknownAttributes match {
      case Nil =>
        ...
        val labelAttributeValue =
          attributeValues.get(orderedRowDescriptionLabelAttributeKey)

        val delimiterAttributeValue =
          attributeValues.get(orderedRowDescriptionDelimiterAttributeKey)

        val rowDescriptionElemChildren =
          XMLHelperFunctions.collectElemChildren(rowDescriptionXmlElem)
        val delimitedColumnDescriptionsAndIdentifiers =
          makeDelimitedColumnIdentifiersAndDelimitedColumnDescriptions(
        ...
        (labelAttributeValue,
         delimiterAttributeValue,
         delimitedColumnDescriptionsAndIdentifiers) match {
          case (Right(label),
                Right(delimiter),
                Right(
                  (delimitedColumnsDescriptionsList,
                   delimitedColumnsIdentifiersList))) =>
            val delimitedRowIdentifierEither =
              DelimitedRowIdentifier(delimitedColumnsIdentifiersList, delimiter)
            delimitedRowIdentifierEither match {
              case Left(errorMessage) =>
                RowToRowDescriptionMatcherErrorMessages
                  .invalidDelimitedRowDescriptionChildElemOrAttribute(
                    errorMessage)
              case Right(delimitedRowIdentifier) =>
                val delimitedRowDivider = DelimitedRowDivider(
                  delimitedColumnsDescriptionsList,
                  delimiter)
                val delimitedRowDescriptions =
                  OrderedRowDescription(delimitedRowDivider, label)
                val rowToRowDescriptionMatcher = RowToRowDescriptionMatcher(
                  delimitedRowIdentifier,
                  delimitedRowDescriptions)
          ...
          case _ =>
            val errorMessages = UnrecoverableError.collectAllErrorsToList(
              labelAttributeValue,
              delimiterAttributeValue,
              delimitedColumnDescriptionsAndIdentifiers)
            RowToRowDescriptionMatcherErrorMessages
              .invalidDelimitedRowDescriptionChildElemOrAttribute(
                errorMessages: _*)
        }
  ...
  def makeFixedRowToRowDescriptionMatcher(rowDescriptionXmlElem: Elem)
    : Either[UnrecoverableError, RowToRowDescriptionMatcher] = {
    ...
    @tailrec def makeFixedColumnIdentifiersAndFixedColumnDescriptions(
        ...
        columnIdentifierList: List[FixedColumnIdentifier]
              ...
              (List[FixedColumnDescription], List[FixedColumnIdentifier])] =
      xmlNodes match {
        case elem :: _
            if !XMLHelperFunctions.elemLabelIsIn(
              elem,
          ...
          RowToRowDescriptionMatcherErrorMessages
            .unknownFixedRowDescriptionChildElem(elem)
        ...
        case elem :: restOfNodes
          ...
          columnDescriptionEither match {
            ...
            case Right(columnDescription) =>
              makeFixedColumnIdentifiersAndFixedColumnDescriptions(
                restOfNodes,
                columnDescription :: columnDescriptionList,
                columnIdentifierList)
          }
    ...
    val unknownAttributes = XMLHelperFunctions.collectUnknownAttributes(
      fixedOrderedRowDescriptionAttributeList,
      rowDescriptionXmlElem.attributes)
    unknownAttributes match {
      case Nil =>
        val rowDescriptionXmlElemChildrenList =
          XMLHelperFunctions.collectElemChildren(rowDescriptionXmlElem)
        val columnIdentifiersAndColumnDescriptions =
          makeFixedColumnIdentifiersAndFixedColumnDescriptions(
            rowDescriptionXmlElemChildrenList,
            Nil,
            Nil)

        val AttributeValues = AttributesValuesExtractor(
          rowDescriptionXmlElem.attributes,
          RowToRowDescriptionMatcher.orderedRowDescriptionLabelAttributeKey)
        val rowDescriptionLabelAttributeValue = AttributeValues.get(
          RowToRowDescriptionMatcher.orderedRowDescriptionLabelAttributeKey)

        (rowDescriptionLabelAttributeValue,
         columnIdentifiersAndColumnDescriptions) match {
          case (Right(label),
                Right((columnDescriptionList, columnIdentifierList))) =>
            ...
            rowIdentifierEither match {
              case Left(errorMessage) =>
                RowToRowDescriptionMatcherErrorMessages
                  .invalidFixedRowDescriptionChildElemOrAttribute(errorMessage)
          ...
          case _ =>
            val errorMessages = UnrecoverableError.collectAllErrorsToList(
              rowDescriptionLabelAttributeValue,
              columnIdentifiersAndColumnDescriptions)
            RowToRowDescriptionMatcherErrorMessages
              .invalidFixedRowDescriptionChildElemOrAttribute(errorMessages: _*)
        }

      case _ =>
        RowToRowDescriptionMatcherErrorMessages
          .invalidFixedRowDescriptionChildElemOrAttribute(unknownAttributes: _*)
    }
  ...
  private val delimitedOrderedRowDescriptionElemLabel =
  ...
  private val orderedRowDescriptionLabelAttributeKey =
    AttributeValueExtractor("label", AttributeValueConversion.extractValue)

  private def delimiterAttributeValueConverter(attribute: Attribute) =
    AttributeValueConversion.extractValueAndConvertTo(LiteralDelimiter(_))(
      attribute)

  private val orderedRowDescriptionDelimiterAttributeKey =
    AttributeValueExtractor(
      "literalDelimiter",
      RowToRowDescriptionMatcher.delimiterAttributeValueConverter(_))

  private val delimitedOrderedRowDescriptionAttributeList = List(
    orderedRowDescriptionLabelAttributeKey,
    orderedRowDescriptionDelimiterAttributeKey)
  private val fixedOrderedRowDescriptionAttributeList = List(
    orderedRowDescriptionLabelAttributeKey)
}
