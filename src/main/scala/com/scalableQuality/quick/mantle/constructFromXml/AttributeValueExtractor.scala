...
class AttributeValueExtractor[ValueType](
  ...
  def valueFrom(attribute: Attribute): Either[UnrecoverableError, ValueType] =
    attributeConverter(attribute)
  def matches(attribute: Attribute): Boolean =
    XMLHelperFunctions.haveKey(attribute, this.key)
}
...
object AttributeValueExtractor {
  ...
  private def defaultToError[ValueType](
      key: String): Either[UnrecoverableError, ValueType] = {
