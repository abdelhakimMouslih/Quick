...
object AttributeValueConversion {
  ...
  def toBoolean(attribute: Attribute): Either[UnrecoverableError, Boolean] =
    extractValueAndConvertTo(
  ...
  def extractValue(attribute: Attribute): Either[UnrecoverableError, String] =
    extractValueAndConvertTo(
      Right(_)
    )(
      attribute
  ...
  def extractValueAndConvertTo[ValueType](
    ...
    extractAttributeValue(attribute) match {
      ...
      case None =>
        AttributeValueConversionErrorMessages.failedToExtractAttributeValue(
          attribute) // dead code
    }
  }
  ...
  private def safePatternCompilation(
      pattern: String): Either[UnrecoverableError, Pattern] =
    Try(Pattern.compile(pattern)) match {
      case Success(pat) => Right(pat)
      case Failure(throwable) =>
        AttributeValueConversionErrorMessages.failedToCompileRegex(pattern,
                                                                   throwable)
    }

  private def safeStringToInt(number: String): Either[UnrecoverableError, Int] =
    Try(number.trim.toInt) match {
      case Success(num) =>
        Right(num)

      case Failure(_) =>
        AttributeValueConversionErrorMessages.failedToConvertStringIntoAnInt(
          number)
    }

  private def safeStringToBoolean(
      boolean: String): Either[UnrecoverableError, Boolean] =
    Try(boolean.trim.toLowerCase.toBoolean) match {
      case Success(bool) =>
        Right(bool)

      case Failure(_) =>
        AttributeValueConversionErrorMessages.failedToConvertStringIntoABoolean(
          boolean)
    }
