...
object ColumnComparisonTableRowFormat{
  ...
  private def formatARow(
    ...
    s"${borders.leftBorder}${labelColumnFormat}${borders.middleBorder}${positionColumnFormat}${borders.middleBorder}${valueColumnFormat}${borders.middleBorder}${valueColumnFormat}${borders.rightBorder}".format(
      row.label,
      row.position,
      row.leftValue,
      row.rightValue
