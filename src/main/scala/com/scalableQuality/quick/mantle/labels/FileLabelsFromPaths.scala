...
object FileLabelsFromPaths {
  ...
  def apply(quickState: QuickState):(Option[String], Option[String]) =
    (quickState.leftFileLabel, quickState.rightFileLabel) match {
      ...
      case (None, Some(_)) =>
        val (leftFileLabel, _) = FileLabelsFromPaths(quickState.leftFile, quickState.rightFile)
        (leftFileLabel, quickState.rightFileLabel)

      case (Some(_), None) =>
        val (_, rightFileLabel) = FileLabelsFromPaths(quickState.leftFile, quickState.rightFile)
  ...
  def apply(
    ...
    (leftFileCanonicalPath, rightFileCanonicalPath) match {
      case (Some(canonicalLeftPath),Some(canonicalRightPath)) =>
        val (leftFileDirs, leftFileName) = splitDirsFromFileName(canonicalLeftPath)
        val (rightFileDirs, rightFileName) = splitDirsFromFileName(canonicalRightPath)
        val (minifiedLeftFileDirs, minifiedRightFileDirs) = minifyDirs(leftFileDirs, rightFileDirs)
