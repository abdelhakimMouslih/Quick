...
import com.scalableQuality.quick.surface.{ExitStatus, FilesAreDifferent, FilesAreIdentical, InterruptedByAnError}

object ExitWithStatus {
  def exitWithReport(filledValidationReports: List[FilledValidationAndMatchingReport] ): Unit = filledValidationReports match {
    case Nil =>
  ...
  private def apply(exitStatus: ExitStatus): Unit = System.exit(exitStatus.value)
}
