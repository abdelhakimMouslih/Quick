...
import com.scalableQuality.quick.core.others.errorMessages.ShouldUseDuringErrorMessages
...
object ShouldUseDuring {
  ...
  def apply(elemMetaData: MetaData): Either[UnrecoverableError, ShouldUseDuring] = {
    ...
    val useDuringReportingParameterValue = attributeValues.get(useDuringReportingKey)
  ...
  private def validateClassParameters(
    ...
    (useDuringValidationParameterValue, useDuringMatchingParameterValue, useDuringReportingParameterValue) match {
      case (Right(useDuringValidation),Right(useDuringMatching),Right(useDuringReporting)) =>
        val classParameters = (useDuringValidation, useDuringMatching, useDuringReporting)
