...
object ValidationProcess {
  ...
  def apply (
    ...
    val leftFileAsHashMap = buildValidationHashMap(orderedRowDescription, leftFileRows)
  ...
  private type validationSignatureHashMap = mutable.HashMap[validationSignature, mutable.Set[RawRow]] with mutable.MultiMap[validationSignature, RawRow]
  ...
  private def buildValidationHashMap(
    ...
    val validationHashMap = new mutable.HashMap[validationSignature, mutable.Set[RawRow]] with mutable.MultiMap[validationSignature, RawRow]
    file.foreach{
      row =>
  ...
  @tailrec private def validate(
    ...
    case rightRow::restOfRightFile =>
      val  rightRowValidationSignature = rowDescription.validationSignatureOf(rightRow)
      val equivalentLeftRowOpt = leftFileAsHashMap.get(rightRowValidationSignature).flatMap(_.headOption)
      equivalentLeftRowOpt match {
        ...
        case Some(equivalentLeftRow) =>
          leftFileAsHashMap.removeBinding(rightRowValidationSignature, equivalentLeftRow)
          validate(
            leftFileAsHashMap,
            rowDescription,
            restOfRightFile,
