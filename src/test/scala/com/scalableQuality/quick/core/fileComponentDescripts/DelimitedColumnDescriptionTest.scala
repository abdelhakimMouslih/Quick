...
class DelimitedColumnDescriptionTest extends FlatSpec with Matchers with GeneratorDrivenPropertyChecks with TableDrivenPropertyChecks {
  ...
  val invalidAttributeValue = Table(
    ("position","useDuringValidation","useDuringMatching","useDuringReporting","trimValue","ignoreValueCase"),
