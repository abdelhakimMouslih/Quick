...
class OrderedRowDescriptionTest extends FlatSpec with Matchers with GeneratorDrivenPropertyChecks {
  ...
  "Delimited OrderedRowDescription.validationSignatureOf" should "return the signature of all 3 validation columns" in {
    ...
    (firstColumnDescriptionEither, secondColumnDescriptionEither, thirdColumnDescriptionEither, delimitedEither) match {
      case (Right(firstColumnDescription),Right(secondColumnDescription),Right(thirdColumnDescription), Right(delimited)) =>
        val columnDescriptionList = List(firstColumnDescription, secondColumnDescription, thirdColumnDescription)
        val delimitedRowDivider = DelimitedRowDivider(columnDescriptionList, delimited)
        val orderedRowDescription = OrderedRowDescription(delimitedRowDivider, "label")
