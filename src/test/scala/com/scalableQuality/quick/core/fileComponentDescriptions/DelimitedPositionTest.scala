...
class DelimitedPositionTest
    ...
    (position: Int) =>
      whenever(position > 0) {
        val columnDescriptionXmlElem =
          <ColumnDescription position={position.toString} />
        val delimitedPositionTest =
          DelimitedPosition(columnDescriptionXmlElem.attributes)
        delimitedPositionTest shouldBe a[Right[_, _]]
      }
  }

  it should "return Left when position is < 1" in forAll { (position: Int) =>
    whenever(position < 1) {
      val columnDescriptionXmlElem =
        <ColumnDescription position={position.toString} />
      val delimitedPositionTest =
  ...
  "DelimitedPosition.extractColumnValue" should
    "return Some(Value) when position is 0 < position <= vector.length" in forAll(
    validPositions) { (position: Int) =>
    val row = Vector("H", "A", "K", "I", "M")
    val columnDescriptionXmlElem =
      <ColumnDescription position={position.toString} />
    val delimitedPositionTestEither =
      DelimitedPosition(columnDescriptionXmlElem.attributes)
