...
class FileLabelsFromPathsTest extends FlatSpec with Matchers {
  ...
  def systemDependentPath(names: String*): String =
    names.mkString(pathSeparator)
  ...
  it should "ignore ./ and return the files name when their parent directory is the same" in {
    ...
    val rightFilePath =
      systemDependentPath(".", "dir", "subDir", "differentFile")
  ...
  it should "remove the common parent directory from the two relative paths" in {
    ...
    val expectedResult = (Some(systemDependentPath("subDir", "file")),
                          Some(systemDependentPath("differentSubDir", "file")))
    FileLabelsFromPaths(leftFilePath, rightFilePath) shouldBe expectedResult
  }
  ...
  it should "treat ../dir... and ./dir as different directories" in {
    ...
    val expectedLeftFilePath =
      Some(systemDependentPath("dir", "subDir", "file"))
    val (obtainedLeftFilePath, obtainedRightFilePath) =
      FileLabelsFromPaths(leftFilePath, rightFilePath)
